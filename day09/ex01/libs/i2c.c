...
void i2c_start(uint8_t address, int mode)
{
    ...
    if (status == TW_MT_SLA_ACK || status == TW_MR_SLA_ACK ||
        status == TW_MT_SLA_NACK || status == TW_MR_SLA_NACK)
    {
        ECHO_STATUS(status, "SLA+W/R transmitted, ACK/NACK received.");
    }
    else
    {
        ERROR(status, "SLA+W/R not transmitted. Exiting...");
        return ;
    }
}
...
uint8_t i2c_read(uint8_t ack)
{
    ...
    {
        ECHO_STATUS(status, "Data received, ACK/NACK received from slave.");
        return TWDR;
    }
    else
    {
        ERROR(status, "Data not received. Exiting...");
        return 0;
    }
}